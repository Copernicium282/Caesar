...
async function updateBadge(tab) {
  ...
  try {
    const data = await browser.storage.session.get("caesar_token");
    const token = data.caesar_token;
    if (!token) {
      browser.action.setBadgeText({ text: "" });
      return;
    }
...
browser.contextMenus.onClicked.addListener(async (info, tab) => {
  if (info.menuItemId === "caesar-fill" && tab?.id) {
    try {
      const data = await browser.storage.session.get("caesar_token");
      const token = data.caesar_token;
  ...
  if (info.menuItemId === "caesar-save" && tab?.id) {
    try {
      const data = await browser.storage.session.get("caesar_token");
      const token = data.caesar_token;
      if (!token || !tab.url) return;
...
function resetIdle() {
  ...
  idleTimer = setTimeout(async () => {
    try {
      const d = await browser.storage.session.get("caesar_token");
      if (d.caesar_token) {
        await fetch(`${API_BASE}/lock`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${d.caesar_token}`,
            "X-Caesar-Client": "1",
          },
        });
...
browser.runtime.onMessage.addListener((msg, sender, sendResponse) => {
    ...
    (async () => {
      try {
        ...
        try {
          ...
          // Basic subdomain match: mail.google.com matches google.com.
          // Does not handle multi-part TLDs (e.g., co.uk) — acceptable for a lightweight extension.
    ...
    (async () => {
      try {
        ...
        const res = await fetch(`${API_BASE}/entries`, {
          ...
          body: JSON.stringify({
            name: (() => { try { return new URL(msg.url).hostname.replace("www.", ""); } catch { return msg.url; } })(),
            username: msg.username,
            password: msg.password,
            url: msg.url,
            uris: [(() => { try { return new URL(msg.url).hostname; } catch { return msg.url; } })()],
          }),
        });
  ...
  if (msg.type === "GET_TOKEN") {
    browser.storage.session
      .get("caesar_token")
      .then((d) =>
        sendResponse({ token: d.caesar_token || null })
      );
    return true;
  }
  if (msg.type === "SET_TOKEN") {
    browser.storage.session
      .set({ caesar_token: msg.token })
